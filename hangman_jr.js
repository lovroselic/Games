...
const MAX_LIVES = 7;
console.log(
  "Hangman JR v" + VERSION + " by Lovro Selic, (c) C00lSch00l 2014, 2015"
);
...
function checkArticle() {
  ...
  if (zmaga) {
    $("#story").html(
      "<img src='" + Source + "/Images/hangman_win.png' width='440'>"
    );
    $("#message").html(
    ...
    $("#story").html(
      "<img src='" + Source + "/Images/hangman0.png' width='440'>"
    );
    $("#theArticle").remove();
    $("#message").html(
...
function checkLetter() {
  ...
  if (whichLett === -1) {
    ...
    lives--;
    $("#story").html(
      "<img src='" + Source + "/Images/hangman" + lives + ".png' width='440'>"
    );
    if (lives === 0) {
      if (article === "none") {
        article = "";
    ...
    if (testEndOfGame === -1) {
      $("#message").html(
        "Message: <span class='green'>Great work. You have guessed the word! Now fill in the correct definite (the) or indefinite (a/an) article to WIN the game.</span>"
      );
      $("#leftovers").html("");
      $("#build").prepend(
        "<input type='text' id='theArticle' size='1' maxlength='3' name='theArticle' />"
      );
...
function colourLetter(character) {
  ...
  while (ix !== -1) {
    colouredSentence[ix] =
      '<span class="blue">' + colouredSentence[ix] + "</span>";
...
function startGame() {
  ...
  setLives();
  $("#story").html(
    "<img src='" + Source + "/Images/hangman7.png' width='440'>"
  );
  var picElement =
    "<img src='" +
    Source +
    "/CoolToolImages/" +
    newWord.toUpperCase() +
    ".png' alt='Image not found' width='145' height='145'>";
